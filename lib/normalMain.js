...
function extractVideo(options) {
    ...
    entryData.forEach((e, i) => {
        ...
        if (fs.existsSync(`${CACHE_PATH}/${dirId}/${e.dirCid}/${e.typeTag}/audio.m4s`) && fs.existsSync(`${CACHE_PATH}/${dirId}/${e.dirCid}/${e.typeTag}/video.m4s`)) {
            ...
            execSync(`mkdir -p "${path.parse(outputFile).dir}" && ${ffmpegPath} -loglevel quiet -i "${CACHE_PATH}/${dirId}/${e.dirCid}/${e.typeTag}/video.m4s" -i "${CACHE_PATH}/${dirId}/${e.dirCid}/${e.typeTag}/audio.m4s" -c copy -movflags faststart -f mp4 "${outputFile}.mp4" ${yes === true ? '-y' : ''} ${danmu === true ? copyDanmu : ''} ${clear === true ? rmDir : ''}`)
